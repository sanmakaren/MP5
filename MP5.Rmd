...
#data imputation 
training <- training %>%
  select(log_SalePrice, MSSubClass,  LotFrontage,  LotArea,  
...
training2.mis <- prodNA(training2, noNA = 0.1)


training2.mis$LotFrontage <- with(training2, impute(LotFrontage, median))
training2.mis$OverallQual <- with(training2, impute(OverallQual, median))
training2.mis$LotArea <- with(training2, impute(LotArea, median))
...
predicted_points_1 <- model_1 %>%
  broom::augment(newdata = training)
predicted_points_1
```

```{r}

min <- min(predicted_points_1$.fitted, na.rm = TRUE)
max <- max(predicted_points_1$.fitted, na.rm = TRUE)
...
```{r}
predicted_points_1 <- mutate(predicted_points_1, .fitted = ifelse(is.na(.fitted),  runif(1, min=min, max=max), .fitted))
```
...
```{r}
min <- min(predict_test$.fitted, na.rm = TRUE)
max <- max(predict_test$.fitted, na.rm = TRUE)
```

```{r}
predict_test <- mutate(predict_test, .fitted = ifelse(is.na(.fitted),  runif(1, min=min, max=max), .fitted))
```
