...
---------------- | ------------------ | -------------
Unregularized lm | 0.003618587        | 0.22466
LASSO            | 0.02440769         | 0.22466


Looking at the two RMLSE values for Unregularized lm and LASSO they both seem that have the same score on kaggle which is 0.22466 but for the training RMLSEs the numbers are a bit different, the RMLSE for the unregularized lm is much closer to zero -- might be we are using all the variables in the model and with LASSO we are dropping variables that don't have much effect on model. 
